...
bool DialogManagerWin::OpenFileDialog(const FString& DialogTitle, const FString& DefaultPath, const FString& FileTypes, bool bMultiSelect, TArray<FString>& OutFilenames)
{
	bool bSuccess = false; 

#if PLATFORM_WINDOWS
	int32 dummy = 0;
	bSuccess = FileDialogShared(false, DialogTitle, DefaultPath, FString(""), FileTypes, bMultiSelect, OutFilenames, dummy);
#endif
	return bSuccess;
}

bool DialogManagerWin::SaveFileDialog(const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, bool bMultiSelect, TArray<FString>& OutFilenames)
{
	bool bSuccess = false; 

#if PLATFORM_WINDOWS
	int32 dummy = 0;
	bSuccess = FileDialogShared(true, DialogTitle, DefaultPath, DefaultFile, FileTypes, bMultiSelect, OutFilenames, dummy);
#endif
    return bSuccess;
}

bool DialogManagerWin::OpenDirectoryDialog(const FString& DialogTitle, const FString& DefaultPath, bool bMultiSelect, TArray<FString>& OutDirectories)
{
	bool bSuccess = false;
	...
	if (SUCCEEDED(::CoCreateInstance(CLSID_FileOpenDialog, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&FileDialog))))
	{
		...
		// Show the picker
		if (SUCCEEDED(FileDialog->Show((HWND)GetOSWindowHandle())))
		{
			if (bMultiSelect)
			{
				...
				{
					auto AddOutFilename = [&OutDirectories](const FString& InFilename)
					{
						FString& OutFilename = OutDirectories[OutDirectories.Add(InFilename)];
			...
			else
			{
				...
				if (SUCCEEDED(FileDialog->GetResult(&Result)))
				{
					...
					if (SUCCEEDED(Result->GetDisplayName(SIGDN_FILESYSPATH, &pFilePath)))
					{
						...
						OutDirectories.Add(OutFolderName);
...
bool DialogManagerWin::FileDialogShared(bool isSaveFileDialog, const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, bool MultipleFiles, TArray<FString>& OutFilenames, int32& OutFilterIndex)
{
	bool bSuccess = false;
