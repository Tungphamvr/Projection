...
bool DialogManagerMac::OpenFileDialog(const FString& DialogTitle, const FString& DefaultPath, const FString& FileTypes, bool bMultiSelect, TArray<FString>& OutFilenames)
{
	int32 dummy = 0;
	return FileDialogShared(false, DialogTitle, DefaultPath, FString(""), FileTypes, bMultiSelect, OutFilenames, dummy);
}

bool DialogManagerMac::SaveFileDialog(const FString &DialogTitle, const FString &DefaultPath, const FString &DefaultFile, const FString &FileTypes, bool bMultiSelect, TArray<FString> &OutFilenames)
{
	int32 dummy = 0;
	return FileDialogShared(true, DialogTitle, DefaultPath, DefaultFile, FileTypes, bMultiSelect, OutFilenames, dummy);
}

bool DialogManagerMac::OpenDirectoryDialog(const FString& DialogTitle, const FString& DefaultPath, bool bMultiSelect, TArray<FString>& OutDirectories)
{
	...
	bool bSuccess = false;
	{
		bSuccess = MainThreadReturn(^{
		  ...
		  [Panel setAllowsMultipleSelection:bMultiSelect];
		  ...
		  NSInteger Result = [Panel runModal];
            ...
            if (Result == NSModalResponseOK)
            {
                ...
                for (NSURL *FolderURL in SelectedURLs)
                {
                    TCHAR FolderPath[MAC_MAX_PATH];
                    FPlatformString::CFStringToTCHAR((CFStringRef)[FolderURL path], FolderPath);
                    FString FolderString(FolderPath);
                    FPaths::NormalizeFilename(FolderString);
                    OutDirectories.Add(FolderString);
                }

                bResult = (OutDirectories.Num() > 0);
            }
...
bool DialogManagerMac::FileDialogShared(bool bSave, const FString &DialogTitle, const FString &DefaultPath, const FString &DefaultFile, const FString &FileTypes, bool MultipleFiles, TArray<FString> &OutFilenames, int32 &OutFilterIndex)
{
